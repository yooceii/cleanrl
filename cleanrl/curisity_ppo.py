...
optimizer = optim.Adam([{'params': agent.parameters()},
                        {'params': icm_model.parameters()}], lr=args.learning_rate, eps=1e-5)
if args.anneal_lr:
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        if not args.use_curiosity:
            ...
            for start in range(0, args.num_envs, num_envs_per_batch):
                ...
                nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                nn.utils.clip_grad_norm_(icm_model.parameters(), args.max_grad_norm)
                # nn.utils.clip_grad_norm_(fwd_model.parameters(), args.max_grad_norm)
                # nn.utils.clip_grad_norm_(inv_model.parameters(), args.max_grad_norm)
                optimizer.step()
